...
/** Extra attribution DSH admits on a producer-owned source (e.g. recall snapshot sections). */
export interface DshProducerSourceFields {
  form?: string;
 ...
 */
export function graphMemorySource(fields: DshProducerSourceFields = {}): Record<string, unknown> {
 ...
 */
export function isGraphMemorySource(source: unknown): boolean {
  if (!source || typeof source !== "object") return false;
  const value = source as { kind?: unknown; plugin?: unknown };
  if (value.kind === DSH_PRODUCER_SOURCE_KIND) return true;
